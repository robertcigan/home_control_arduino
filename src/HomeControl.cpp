...
void HomeControl::loop() {
  ...
  //   static uint32_t clientConnectedSince = 0;
  //   static uint32_t connectionExpiredSince = 0;
  //   static uint32_t wifiStatusSince = 0;
  //   static bool lastClientConnected = false;
  //   static bool lastConnectionExpired = false;
  //   static int lastWifiStatus = -1;

  //   // Track status changes
  //   bool currentClientConnected = client.connected();
  //   bool currentConnectionExpired = connectionExpired();

  //   if (currentClientConnected != lastClientConnected) {
  //     clientConnectedSince = millis();
  //     lastClientConnected = currentClientConnected;
  //   }

  //   if (currentConnectionExpired != lastConnectionExpired) {
  //     connectionExpiredSince = millis();
  //     lastConnectionExpired = currentConnectionExpired;
  //   }

  //   #if defined(WITH_WIFI)
  //   int currentWifiStatus = WiFi.status();
  //   if (currentWifiStatus != lastWifiStatus) {
  //     wifiStatusSince = millis();
  //     lastWifiStatus = currentWifiStatus;
  //   }
  //   #endif

  //   if (millis() - lastDebugOutput > 1000) { // Output debug info every second
  //     uint32_t currentTime = millis();
  //     uint32_t clientTime = (currentTime - clientConnectedSince) / 1000; // in seconds
  //     uint32_t expiredTime = (currentTime - connectionExpiredSince) / 1000; // in seconds

  //     Serial.print(F("Debug - Client connected: "));
  //     Serial.print(currentClientConnected ? F("YES") : F("NO"));
  //     Serial.print(F(" ("));
  //     Serial.print(clientTime);
  //     Serial.print(F("s)"));

  //     Serial.print(F(", Connection expired: "));
  //     Serial.print(currentConnectionExpired ? F("YES") : F("NO"));
  //     Serial.print(F(" ("));
  //     Serial.print(expiredTime);
  //     Serial.print(F("s)"));

  //     #if defined(WITH_WIFI)
  //       uint32_t wifiTime = (currentTime - wifiStatusSince) / 1000; // in seconds
  ...
  //         default:               Serial.print(currentWifiStatus); break;
  //       }
  //       Serial.print(F(" ("));
